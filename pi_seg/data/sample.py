...
from pi_seg.utils.misc import get_labels_with_sizes
...
class DSample:
    ...
    def augment(self, augmentator):
        ...
        aug_output = {'input': self.image, 'label': self._encoded_masks}
        
        if augmentator:
            aug_output = augmentator(aug_output)

        image = aug_output['input'].to('cpu').detach().numpy().transpose(1, 2, 0)
        mask = aug_output['label'].to('cpu').detach().numpy().astype(np.int32).transpose(1, 2, 0)
