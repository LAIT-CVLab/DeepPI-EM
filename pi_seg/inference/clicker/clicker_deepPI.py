...
class Clicker(object):
    ...
    def make_next_click(self, pred_mask, uncertainty_map=None, top_uncertainty=True):
        assert self.gt_mask is not None
        num_clicks = len(self.clicks_list)
        click = None
        if num_clicks == 0:
            click = self._get_first_click(pred_mask)
        else:
            click = self._get_next_click(pred_mask, uncertainty_map=uncertainty_map, top_uncertainty=top_uncertainty)
        self.add_click(click)

    def get_clicks(self, clicks_limit=None):
        return self.clicks_list[:clicks_limit]
    
    def _get_first_click(self, pred_mask, padding=True, uncertainty_map=None):
        pred_mask = pred_mask[0][0].cpu().detach().numpy()
    ...
    def _get_next_click(self, pred_mask, padding=True, uncertainty_map=None, top_uncertainty=True): 
        tmp_uncertainty_map = uncertainty_map[0][0].cpu().detach().numpy()
        
        max_index = np.where(tmp_uncertainty_map == np.max(tmp_uncertainty_map))
        min_index = np.where(tmp_uncertainty_map == np.min(tmp_uncertainty_map))
        ...
        if top_uncertainty == False:
            coords = (min_x, min_y)

        is_positive = self.gt_mask[coords[0], coords[1]] == True

        return Click(is_positive=is_positive, coords=coords)
